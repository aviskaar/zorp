...
- `zorp-recall/` is zorp's own conversation search: a loopback guard, an
  ...
  serializes every pass, runs outside server startup and turns, relies on the
  existing fingerprint skip, and logs a repeated failure only when its state
  changes. The Index button is gone, but `POST /api/recall/index` remains for
  tests and scripts that need to force a pass.
  ...
  `docs/DECISIONS.md` (2026-08-22, 2026-08-24) before changing any of that, especially
