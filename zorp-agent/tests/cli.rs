...
fn unknown_leading_long_flag_is_usage_error() {
    let out = Command::new(bin())
        ...
        .unwrap();
    assert_eq!(out.status.code(), Some(2));
    assert!(String::from_utf8_lossy(&out.stderr).contains("Usage: zorp-agent"));
}
...
fn chat_help_and_exit_without_model() {
    ...
    let mut child = Command::new(bin())
        ...
        .env("ZORP_STATE_DB", dir.path().join("s.db"))
        .env("ZORP_MODEL", "m")
        .env("ZORP_BASE_URL", "http://127.0.0.1:1") // unused: no plain-text turn
        .env_remove("ZORP_API_KEY")
...
fn untrusted_project_verify_is_not_applied_noninteractively() {
    ...
    let out2 = Command::new(bin())
        ...
        .current_dir(dir.path())
        .env("HOME", dir.path())
        .env("ZORP_BASE_URL", &base2)
        .env("ZORP_MODEL", "m")
        .env("ZORP_STATE_DB", dir.path().join("s2.db"))
        .env("ZORP_TRUST_FILE", dir.path().join("trust2"))
        .env_remove("ZORP_API_KEY")
        .stdin(std::process::Stdio::null())
        .output()
        .unwrap();
...
fn new_scaffolds_a_manifest_starter() {
    ...
    let path = dir.path().join(".zorp/flavors/reviewer.toml");
