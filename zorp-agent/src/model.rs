...
/// The real model client: buffered `zorp_raw` against an OpenAI-compatible endpoint.
#[derive(Clone)]
...
impl HttpModel {
    ...
    /// Use [`HttpModel::try_from_env`] to also validate and apply `ZORP_REASONING_MODE`.
    pub fn from_env() -> Self {
        let (base, key, model, _system) = zorp::env_config();
        HttpModel {
            url: zorp::join_url(&base, "chat/completions"),
            api_key: key,
            model,
    ...
    /// Build from all model environment settings, including `ZORP_REASONING_MODE`.
...
impl Model for HttpModel {
    ...
    fn complete_with_options(
        ...
        let span = tracing::span!(
            tracing::Level::INFO,
            "model_complete",
            zorp.model = self.model.as_str(),
            zorp.messages_sent = messages.len(),
            zorp.tools_provided = tools.len(),
            zorp.requested_reasoning_mode = tracing::field::Empty,
            zorp.provider_reasoning_parameters = tracing::field::Empty,
            zorp.reasoning_parameters_sent = tracing::field::Empty,
            zorp.reasoning_content_available = tracing::field::Empty,
            zorp.actual_reasoning_tokens = tracing::field::Empty
        );
        #[cfg(feature = "otel")]
            ...
            .provider
        {
            crate::provider::Provider::OpenAiCompatible => {
                ...
                let resp = zorp::zorp_raw(&self.url, &headers, body)?;
                (
...
mod tests {
    ...
    fn try_from_env_rejects_invalid_reasoning_mode() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _env = EnvGuard::set(&[("ZORP_REASONING_MODE", Some("turbo"))]);
