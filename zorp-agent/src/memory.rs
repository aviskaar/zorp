...
//!
//! It lives in `zorp-agent` because both surfaces read the same index the
//! same way. It was in `zorp-web`, which meant a terminal turn could not
//! recall anything, and copying it would have given the workspace two
//! fences, two nonces and two boundary sentences over the same untrusted
//! text.
//!
