...
//!
//! It lives in `zorp-agent` rather than in `zorp-recall` because
//! `zorp-recall` deliberately depends on no other workspace member, the way
//! `zorp-search` and `zorp-skill` do, and making it depend on `zorp-agent`
//! would invert that and pull the whole agent into an embedding crate. The
//! store is here, and the store is what the chunker reads.
//!
//! It was in `zorp-web`, which meant the terminal could not search its own
//! conversations at all even though the index is over a store both surfaces
//! share. Copying it would have given the workspace two chunkers and two
//! fingerprints, and the day those disagreed the index would quietly hold
//! two conventions.
//!
//! # What gets embedded
//!
...
pub fn index_path() -> PathBuf {
    ...
    let sessions = crate::Store::default_path();
...
fn reindex_unlocked() -> Result<Report, RecallError> {
    ...
    reindex_paths(&crate::Store::default_path(), &index_path(), &embedder)
}
...
fn reindex_paths(
    ...
    let store = crate::Store::open_at(store_path).map_err(|e| RecallError::Store(e.to_string()))?;
    let sessions = store
        .sessions()
        .map_err(|e| RecallError::Store(e.to_string()))?;
...
fn feed_session_unlocked(session_id: &str) -> Result<Report, RecallError> {
    ...
    let store = crate::Store::open_default().map_err(|e| RecallError::Store(e.to_string()))?;
    // One indexed row, not every conversation in the store walked to find
    // one. This runs on every finished turn.
    let session = store
        .session(session_id)
        .map_err(|e| RecallError::Store(e.to_string()))?;
...
///
/// What `zorp-web`'s memory and the CLI's `--recall` read. Same index and same embedder as `search`,
...
fn chunks_for(messages: &[crate::Message]) -> Vec<Chunk> {
    messages
        .iter()
        .enumerate()
...
mod tests {
    ...
    fn seed(store_path: &Path) {
        let mut store = crate::Store::open_at(store_path).unwrap();
        store
        ...
        store
            .record_message(
                "conv-1",
                0,
                &crate::Message::user("A long enough first message to be embedded"),
            )
            .unwrap();
        store
            .record_message(
                "conv-1",
                1,
                &crate::Message::assistant("A long enough answer to be embedded too"),
            )
            .unwrap();
    }
    ...
    fn a_changed_conversation_is_picked_up_by_the_next_sweep() {
        ...
        crate::Store::open_at(&store_path)
            .unwrap()
            .record_message(
                "conv-1",
                2,
                &crate::Message::user("A later correction changes this conversation"),
            )
            .unwrap();
