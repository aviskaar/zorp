...
impl TraceIdentity {
    pub fn from_env() -> Self {
        TraceIdentity {
            experiment_id: std::env::var("ZORP_EXPERIMENT_ID").ok(),
            task_id: std::env::var("ZORP_TASK_ID").ok(),
            runtime_id: std::env::var("ZORP_RUNTIME_ID").ok(),
            run_id: std::env::var("ZORP_RUN_ID").ok(),
            repetition: std::env::var("ZORP_REPETITION")
                .ok()
                .and_then(|s| s.parse().ok()),
            zorp_commit: std::env::var("ZORP_COMMIT").ok(),
            snapshot_hash: std::env::var("ZORP_SNAPSHOT_HASH").ok(),
        }
    }
}
...
impl Agent {
    ...
    pub fn new(
        ...
        let trace_file = std::env::var("ZORP_TRACE_FILE").ok().and_then(|path| {
    ...
    pub fn run_multimodal(&mut self, parts: Vec<ContentPart>) -> Outcome {
        ...
        let span = tracing::span!(
            tracing::Level::INFO,
            "agent_run",
            zorp.task = redacted_task.as_str(),
            zorp.max_steps = self.max_steps
        );
        #[cfg(feature = "otel")]
        ...
        let outcome = loop {
            ...
            for call in &msg.tool_calls {
                ...
                let tool_span = tracing::span!(
                    tracing::Level::INFO,
                    "tool_execute",
                    zorp.tool_name = call.name.as_str(),
                    zorp.tool_arguments = %redacted_args,
                    zorp.tool_summary = tracing::field::Empty
                );
                #[cfg(feature = "otel")]
                ...
                #[cfg(feature = "otel")]
                {
                    tool_span.record("zorp.tool_summary", &out.summary);
