//! A short, model-written name for a conversation.
//!
//! A list that shows `sessions.task`, the first message the user typed and
//! cut off wherever the column ran out, is not a list of conversations when
//! they all begin "hello". This asks the session's own model for a title
//! once, the first time a session has both a question and an answer in it.
//!
//! It lives in `zorp-agent` because both surfaces write the column. It was
//! in `zorp-web`, which meant a conversation started in the terminal never
//! got a name, including later in the browser sidebar, where it showed its
//! raw first message forever. Everything here needs `Message`, `Model` and
//! `Store` and nothing else, so the only thing that stayed behind is the
//! part that resolves a model out of the browser's saved settings.
//!
