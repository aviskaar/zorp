        ...
        for runtime in &runtimes {
            for repetition in 0..manifest.experiment.repetitions {
                ...
                cmd.current_dir(&task_dir)
                    ...
                    .env("ZORP_EXPERIMENT_ID", &manifest.experiment.id)
                    .env("ZORP_TASK_ID", &task_id)
                    .env("ZORP_RUNTIME_ID", &runtime.id)
                    .env("ZORP_RUN_ID", &run_id)
                    .env("ZORP_REPETITION", repetition.to_string())
                    .env("ZORP_SNAPSHOT_HASH", &snapshot_hash)
                    .env("ZORP_REASONING_MODE", &runtime.reasoning_mode);
                if let Some(provider) = &runtime.provider {
                    cmd.env("ZORP_PROVIDER", provider);
                }
                if let Some(model) = &runtime.model {
                    cmd.env("ZORP_MODEL", model);
                }
                if let Some(base_url) = &runtime.base_url {
                    cmd.env("ZORP_BASE_URL", base_url);
                }
                if let Some(api_key_env) = &runtime.api_key_env {
                    if let Ok(api_key) = std::env::var(api_key_env) {
                        cmd.env("ZORP_API_KEY", api_key);
                    }
                }
                if let Some(allowed_paths) = task_allowed_paths(&task_dir)? {
                    cmd.env("ZORP_ALLOWED_PATHS", allowed_paths);
                }
...
mod tests {
    ...
    fn run_suite_passes_provider_model_and_allowed_paths_to_agent() {
        ...
        fs::write(
            &fake_agent,
            "#!/bin/sh\n\
             [ \"$ZORP_PROVIDER\" = \"anthropic\" ] || { echo BAD_PROVIDER >&2; exit 1; }\n\
             [ \"$ZORP_MODEL\" = \"claude-sonnet-5\" ] || { echo BAD_MODEL >&2; exit 1; }\n\
             [ \"$ZORP_ALLOWED_PATHS\" = \"backend/**\" ] || { echo BAD_SCOPE >&2; exit 1; }\n\
             echo '{\"event_type\":\"run.start\",\"seq\":0}' >> \"$ZORP_TRACE_FILE\"\n\
             exit 0\n",
        )
        .unwrap();
