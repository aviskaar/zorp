...
use crate::protocol::{JsonRpcNotification, JsonRpcRequest, JsonRpcResponse, McpTool, mcp_prefix, PROTOCOL_VERSION};
...
impl McpServer {
    ...
    pub fn initialize(&mut self) -> Result<(), McpError> {
        ...
        self.transport.send_notification(JsonRpcNotification::new("notifications/initialized", None))?;
        Ok(())
    }
    ...
    pub fn list_tools(&mut self) -> Result<Vec<McpTool>, McpError> {
        ...
        loop {
            ...
            let resp = self.transport.send(JsonRpcRequest::new(id, "tools/list", params))?;
            if let Some(err) = resp.error {
                return Err(McpError::ServerError { code: err.code, message: err.message });
            }
...
mod tests {
    ...
    fn initialize_request_has_correct_method() {
        let req = JsonRpcRequest::new(1, "initialize", Some(json!({"protocolVersion":PROTOCOL_VERSION,"capabilities":{},"clientInfo":{"name":"zorp-mcp","version":"0.1.0"}})));
    ...
    fn err_resp(id: Value, code: i64, message: &str) -> JsonRpcResponse {
        serde_json::from_value(json!({"jsonrpc":"2.0","id":id,"error":{"code":code,"message":message}})).unwrap()
    }
    ...
    impl Transport for MockTransport {
        fn send(&mut self, req: JsonRpcRequest) -> Result<JsonRpcResponse, McpError> {
            ...
            match req.method.as_str() {
                "initialize" => Ok(ok_resp(req.id, json!({
                    "protocolVersion": PROTOCOL_VERSION,
                    "capabilities": {},
                    "serverInfo": {"name": "mock", "version": "0"}
                }))),
                "tools/list" => {
                    if st.require_initialized && !st.notified.iter().any(|m| m == "notifications/initialized") {
                        return Ok(err_resp(req.id, -32002, "server not initialized"));
                    }
                    let page: usize = req.params.as_ref()
                ...
                other => Ok(err_resp(req.id, -32601, &format!("method not found: {other}"))),
            }
        }
    ...
    fn tools_list_follows_next_cursor_across_pages() {
        ...
        let st = state.lock().unwrap();
        let list_calls: Vec<&Option<Value>> = st.sent.iter()
            .filter(|(m, _)| m == "tools/list")
            .map(|(_, p)| p)
            .collect();
        assert_eq!(list_calls.len(), 2);
        assert!(list_calls[0].is_none(), "first page should not send a cursor");
    ...
    fn tools_list_repeated_cursor_errors_instead_of_looping() {
        ...
        assert!(err.to_string().contains("repeated cursor"), "unexpected error: {err}");
    }
}
