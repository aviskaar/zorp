        ...
        for cfg in &config.servers {
            match McpServer::from_config(cfg) {
                Ok(mut srv) => match srv.initialize() {
                    Ok(()) => servers.push(srv),
                    Err(e) => eprintln!("zorp-mcp: warning: server '{}' initialize failed: {e}", cfg.name),
                },
                Err(e) => eprintln!("zorp-mcp: warning: could not connect to '{}': {e}", cfg.name),
            }
        }
        McpRegistry { servers }
    }
    ...
    pub fn discover(&mut self) -> Vec<McpTool> {
        ...
        for srv in &mut self.servers {
            match srv.list_tools() {
                ...
                Err(e) => eprintln!("zorp-mcp: warning: tools/list on '{}': {e}", srv.name),
            }
        }
        all
    }
            ...
            match srv.list_prompt_names() {
                Ok(names) => for name in names {
                    match srv.get_prompt(&name) {
                        ...
                        Ok(_) => {}
                        Err(e) => eprintln!("zorp-mcp: prompts/get '{name}': {e}"),
                    }
                },
                Err(e) => eprintln!("zorp-mcp: prompts/list on '{}': {e}", srv.name),
            }
        }
        additions
    }
