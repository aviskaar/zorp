 ...
 * careful, so it is never on quietly: a pill in the toolbar, which is on
 * screen whatever the user has scrolled to, says so and is the switch that
 * puts it back. Every call it lets through is also reviewed before it runs;
 * see `zorp-web/src/tool_safety.rs`.
 *
...
/** The elements that say what the session is doing. */
 ...
 * server owns this flag; a page that drew the pill from a local guess could
...
export function renderAutoApprove(view: AutoApproveView, on: boolean): void {
  ...
  view.button.title = on
    ? "Tools are running without asking, after a safety check. Click to start asking again."
